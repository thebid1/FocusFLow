...
### PWA / Notifications
- **vite-plugin-pwa** generates the web app manifest and a single combined service worker (`src/sw.ts` → `dist/sw.js`, served at `/sw.js`)
- The **combined service worker** handles both Workbox precaching/runtime caching and FCM push/notification-click events
- **App owns the root of its own subdomain** (`app.korgix.xyz`): Vite `base` is the default `/`, and the SW scope and manifest `start_url`/`scope` are all `/`. The marketing landing page is a separate Vercel project deployed from `appsite/` at `korgix.xyz`. Keep every absolute app path (`/sw.js`, `/icons/...`) consistent with this.
...
- **PWA shortcuts & share target**: `/?action=add` opens the add-task view; the OS share target can pre-fill title/text/url
...
├── vercel.json               # Vercel deployment settings for the app project (builds to dist/)
├── appsite/                  # Standalone marketing landing page (self-contained; its own Vercel project at korgix.xyz)
...
- **URL layout:** Two Vercel projects deploy from this one repo, split by Root Directory:
  - **App** — Root Directory is the repo root; builds to `dist/` (Vite default `base: '/'`) and serves the PWA at the root of `app.korgix.xyz`. `vercel.json` redirects the legacy `/app/*` path to `/` for old bookmarks.
  - **Landing** — Root Directory is `appsite/`; static files served as-is at `korgix.xyz`, no build step.
- **Landing SW shim:** `appsite/sw.js` unregisters the old root-scoped service worker left by installs from before the app moved to `/app/` — it clears caches, unregisters itself, and reloads open clients. It is served at the root of `korgix.xyz`; the app's own worker is a separate origin and never interacts with it.
- **Firebase authorized domains:** `korgix.xyz` and `app.korgix.xyz` must both be listed under Authentication → Settings → Authorized domains, or sign-in fails.
...
- PWA manifest and Workbox caching are generated by `vite-plugin-pwa` at build time from `src/sw.ts` to `dist/sw.js`; `main.tsx` registers `/sw.js` manually
